...
const courseId = "level-1-math-ii-physics";
const courseRoot = path.join(
  root,
  "src/lib/docs/level-1-math-ii-physics/exams",
);

const expectedPapers = Array.from({ length: 8 }, (_, index) => {
  const setNumber = index + 1;

  return {
    file: `set-${setNumber}/paper-1.json`,
    importName: `set${setNumber}Paper1`,
    setId: `set-${setNumber}`,
  };
});
...
const requiredQuestionMarks = [14, 14, 14, 14, 14, 15, 15];
...
function validatePaper(paper, expected) {
  ...
  if (!Array.isArray(paper.instructions) || paper.instructions.length < 4) {
    fail(`${expected.file} must have exam instructions`);
  }
  ...
  paper.questions.forEach((question, questionIndex) => {
    ...
    if (question.marks !== requiredQuestionMarks[questionIndex]) {
      fail(
        `${expected.file} question ${question.id} must be worth ${requiredQuestionMarks[questionIndex]} marks`,
      );
    }
    ...
    if (
      ...
      fail(`${expected.file} question ${question.id} needs two to five parts`);
    }

    const partMarks = question.parts.reduce((sum, part) => sum + part.marks, 0);

    if (partMarks !== question.marks) {
    ...
    question.parts.forEach((part) => {
      ...
      if (typeof part.marks !== "number" || part.marks <= 0) {
        fail(`${expected.file} part ${part.id} needs positive marks`);
      }
    });
  });
...
if (actualFiles.join("\n") !== expectedFiles.join("\n")) {
  fail(
    `Expected Math II exam JSON files ${expectedFiles.join(", ")}, got ${actualFiles.join(", ")}`,
  );
}
...
const examIndexPath = path.join(courseRoot, "index.ts");
const examIndexSource = fs.readFileSync(examIndexPath, "utf8");
...
console.log("Level 1 Math II exam papers are valid and integrated.");
